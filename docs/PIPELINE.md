...
- **failure**: `attempts + 1`, retried after 1m, then 4m; on the third failure the row moves to `dead_letters` with its error, so blocks and 404s are inspectable instead of retried forever.
